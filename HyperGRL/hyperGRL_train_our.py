...
def edge_prediction_linear_eval(args,edge_split,embeds,data):
    lr=0.001
    max_epoch=getattr(args, 'edge_epochs', 1000)
    h_dim=512
    ...
    valid_score=0
    best_epoch=0
    # Keep a valid classifier state even when the first validation pass is
    # NaN/invalid; otherwise the formal EP runner fails after training.
    param=copy.deepcopy(classifier.state_dict())
    valid_results=(0.0, 0.0, 0.0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', default="pubmed_cite")

    parser.add_argument('--hidden_dim', type=int, default=128,
                        help='Number of hidden units.')
    parser.add_argument('--batch_size', type=int, default=64,
                        help='Graphs per DGL batch during training/evaluation.')
    ...
    if True: #node
        ...
        for seed in range(args.num_seeds):
            ...
            Y_pre_train = [Y_pre[j] for j in train_idx]

            X_vali = [X[j] for j in valid_idx]
            Y_vali = [Y[j] for j in valid_idx]
            Y_pre_vali = [Y_pre[j] for j in valid_idx]

            X_test = [X[j] for j in test_idx]
            Y_test = [Y[j] for j in test_idx]
            Y_pre_test = [Y_pre[j] for j in test_idx]
            ...
            if model_to_use == 'hyperedge_clique' or model_to_use == 'hyperedge_tree':
                scorer.train(dname=args.data,g_list_train=X_train, labels_train_tune=Y_train, labels_train_pre=Y_pre_train, g_list_validation=X_vali,
            ...
            # for node in X:
            #     _ , embed = scorer.predict_labels(node,dname=args.data)
            #     node_i=th.from_numpy(embed)
            #     print(node_i)
            # path=f"./HyperGRL/embeds/HyperGRL_{dname}"
            # th.save(graph_predicted_proba,path)
        ...
        with open(path, 'a+') as write_obj:
            write_obj.write(f'{args.data},{args.method},{test.mean()*100:.1f} ± {test.std()*100:.1f}\n')
