...
class GATClassifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, class_num, dname,num_nodes,dropout_rate = 0.5, num_heads = 5, cluster_num=20, n_negative=10):
    ...
    def forward(self, g):
        ...
        g_size = [graph.number_of_nodes() for graph in g_list]
        h = g.ndata['node_attr']
        ...
        for i in range(len(g_list)):
            ...
            sim=th.dot(h_selected_context, h_selected_node)
            if th.isnan(sim).any():
                sim=th.tensor(0,device=sim.device)
            product_list.append(sim)
            counter += g_size[i]
            for j in range(self.n_negative):
                ...
                if i != rand:
                    ...
                    sim=th.dot(h_n_node, h_selected_context) / (th.norm(th.tensor(h_n_node)) *
        ...
        path=f"./HyperGRL/embeds/HyperGRL_{self.dname}"
        th.save(hg,path)
        return self.classify_graph(hg), self.classify_graph_to_cluster(hg), self.distinguisher(products), label_pre_node

    def encode(self, g):
        """Return one learned graph/node embedding per input graph.

        HyperGRL represents each original node as a clique graph.  Keeping the
        pooled representation available lets the EP runner reuse the trained
        encoder instead of silently falling back to raw features.
        """
        h = g.ndata['node_attr']
        ...
        with g.local_scope():
            g.ndata['result'] = hi
            return self.mean_pool(g, g.ndata['result'])
...
class HyperScorerGeneral(HyperGNNScorerGeneral):
    ...
    def __init__(self, input_dim= 0, num_class=0, thresholds = None, hidden_size=64, n_epochs=1, dropout=0.5, n_heads = 5,
                 thresholds_edge= None, cluster_num=20, batch_size=64, weight_tune=None, weight_pre=None, n_negative=10, lr=0.01,device="cuda:1",dname="citeseer_cite",num_nodes=2708):
        ...
        if weight_tune:
            self._loss_func_tune = nn.CrossEntropyLoss() #(weight=th.tensor(weight_tune).to(self.device))
        else:
            self._loss_func_tune = nn.CrossEntropyLoss()
        if weight_pre:
            self._loss_func_pre = nn.CrossEntropyLoss() #(weight=th.tensor(weight_pre).to(self.device))
        else:
        ...
        self._scheduler = optim.lr_scheduler.ReduceLROnPlateau(self._optimizer, 'min', patience=3, factor=0.2, min_lr=10 ** -5)
