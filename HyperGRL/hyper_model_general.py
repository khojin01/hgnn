...
class HyperGNNScorerGeneral(GraphScorer):
    ...
    def fit(self, train_dataloader, valid_dataloader, optimizer, loss_func_tune, loss_func_pre,dname, n_epochs= 100, alpha=1, scheduler=None,
        ...
        def train(model, iterator, optimizer, loss_func_tune, loss_func_pre, alpha, tuning=False, train_mode=train_mode,
            ...
            for iter, (bg, label_tune, label_self) in enumerate(iterator):
                label_tune = label_tune.float().to(bg.device)
                label_self = label_self.float().to(bg.device)
        ...
        def evaluate(model, iterator, loss_func_tune, loss_func_pre, alpha, tuning=False, train_mode=train_mode,
            ...
            with th.no_grad():
                for iter, (bg, label, label_self) in enumerate(iterator):
                    label = label.float().to(bg.device)
                    label_self = label_self.float().to(bg.device)
        ...
        if pretrain_node:
            ...
            with open(path, 'a+') as write_obj:
                write_obj.write(f'HyperGRL1_{end_time-start_time}\n')
        if pretrain_he or joint or train_mode == 'separate':
            ...
            for epoch in tqdm(range(n_epochs)):
                ...
                scheduler2 = optim.lr_scheduler.ReduceLROnPlateau(optimizer2, 'min', patience=3, factor=0.2,
                                                                  min_lr=10 ** -5)
            ...
            path=f'./time/time.txt'
            with open(path, 'a+') as write_obj:
                write_obj.write(f'HyperGRL2_{end_time-start_time}\n')
        if train_mode == 'pretrain':
    ...
    def train(self, dname,g_list_train, labels_train_tune, labels_train_pre, g_list_validation, labels_validation_tune,
        ...
        self.fit(train_dataloader, valid_dataloader, self._optimizer, self._loss_func_tune, self._loss_func_pre,dname,
    ...
    def predict_labels(self, g_list):
       ...
       """
        ...
        return predicted_labels#,graph_predicted_proba
