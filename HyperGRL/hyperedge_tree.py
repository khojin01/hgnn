"""Pet-GNNH with tree expansion using GAT."""
...
class GATClassifier(nn.Module):
    def __init__(self, in_dim, hidden_dim, class_num, dropout_rate = 0.5, num_heads = 5, cluster_num=20, n_negative=10):
        ...
        self.pool_graph = Set2Set(in_dim, 1, 1)
        self.mean_pool = AvgPooling()
        self.n_negative = n_negative

        # Output a single output class
        self.classify_graph = nn.Sequential(nn.Linear(in_dim * 2, int(in_dim)), nn.ReLU(), nn.Dropout(0.5),
                                            nn.Linear(int(in_dim), self.class_num), nn.Softmax(dim=1))

        # Use regression model in dis2cluster pretext task
        self.regression_graph = nn.Linear(hidden_dim * 2, cluster_num)

        # Use classification model in dis2cluster pretext task
        self.classify_graph_to_cluster = nn.Sequential(nn.Linear(in_dim *2, int(in_dim)), nn.ReLU(), nn.Dropout(0.5),
                                                       nn.Linear(int(in_dim), self.cluster_num), nn.Softmax(dim=1))
    ...
    def forward(self, g):
        ...
        h = g.ndata['node_attr']
        hg = self.pool_graph(g, g.ndata['node_attr'])
...
class HyperScorerGeneral(HyperGNNScorerGeneral):
    ...
    def __init__(self, input_dim= 0, num_class=0, thresholds = None, hidden_size=64, n_epochs=1, dropout=0.5, n_heads = 5,
        ...
        self._scheduler = optim.lr_scheduler.ReduceLROnPlateau(self._optimizer, 'min', patience=3, factor=0.2,
                                                               min_lr=10 ** -5)
