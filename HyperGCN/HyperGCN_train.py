...
if __name__ == '__main__':
    ...
    for seed in tqdm(range(args.num_seeds)):
        ...
        if args.task=="edge":
            ...
            split_hypergraph={}
            for i in range(len(data.hyperedge_index[1])):
                if data.hyperedge_index[1][i].item() in split_hypergraph:
                    split_hypergraph[data.hyperedge_index[1][i].item()].append(data.hyperedge_index[0][i].item())
                
                else:
                    split_hypergraph[data.hyperedge_index[1][i].item()]=[data.hyperedge_index[0][i].item()]
            data.hypergraph = {k: v for k, v in split_hypergraph.items() if len(v) != 1}
            data.num_nodes = int(data.hyperedge_index[0].max()) + 1
            data.num_edges = int(data.hyperedge_index[1].max()) + 1

            criterion = nn.BCELoss()
            eval_func = eval_acc_edge
            model = HyperGCN_e(V=data.features.shape[0],
                         X=data.features,args=args
                         ).to(args.device)
        ...
        if args.task=="node":
            ...
            model = HyperGCN_n(V=data.features.shape[0],
                            E=data.hypergraph,
                            X=data.features,args=args
                            ).to(args.device)
            optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-6)
