...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('MLP model')
    ...
    for seed in tqdm(range(args.num_seeds)):
        ...
        if args.task=="edge":
            ...
            model = MLP_model_e(args).to(args.device)
            ...
            for epoch in range(args.epoch):
                model.train()
                optimizer.zero_grad()
                out=model(data.features,train_v,train_e)
                if out.size(-1) != 1:
                    assert False
            ...
            model = MLP_model_n(args).to(args.device)
            optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-6)
            best_val = float('-inf')
            import time
            for epoch in range(args.epoch):
