...
#         ipdb.set_trace()
        if aggr is None:
            raise ValeuError("aggr was not passed!")
        return scatter(inputs, index, dim=self.node_dim, reduce=aggr)
class SetGNN_e(nn.Module):
    def __init__(self, args, norm=None):
        super(SetGNN_e, self).__init__()
        ...
        if self.All_num_layers == 0:
            ...
            self.V2EConvs.append(HalfNLHconv(in_dim=args.num_features,
                                             ...
                                             heads=args.heads,
                                             attention=args.PMA))
            self.bnV2Es.append(nn.BatchNorm1d(self.MLP_hidden))
            self.E2VConvs.append(HalfNLHconv(in_dim=self.MLP_hidden,
                                             hid_dim=self.MLP_hidden,
                                             out_dim=self.MLP_hidden,
                                             num_layers=self.MLP_num_layers,
                                             dropout=self.dropout,
                                             ...
                                             heads=args.heads,
                                             attention=args.PMA))
            self.bnE2Vs.append(nn.BatchNorm1d(self.MLP_hidden))
            for _ in range(self.All_num_layers-1):
                self.V2EConvs.append(HalfNLHconv(in_dim=self.MLP_hidden,
                                                 hid_dim=self.MLP_hidden,
                                                 out_dim=self.MLP_hidden,
                                                 num_layers=self.MLP_num_layers,
                                                 dropout=self.dropout,
            ...
            if self.GPR:
                ...
                self.classifier = MLP(in_channels=self.MLP_hidden,
                                      hidden_channels=self.Classifier_hidden,
                                      out_channels=self.MLP_hidden,
                                      num_layers=self.Classifier_num_layers,
                                      dropout=self.dropout,
...
#             data.V2Eedge_index[0] contains nodes and data.V2Eedge_index[1] contains hyperedges
        ...
        x, edge_index, norm = data.features, data.hyperedge_index, data.norm
        if self.LearnMask:
...
class SetGNN_n(nn.Module):
    def __init__(self, args, norm=None):
        super(SetGNN_n, self).__init__()
        """
