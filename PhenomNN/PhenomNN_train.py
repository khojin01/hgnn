...
if __name__ == '__main__':
    ...
    parser.add_argument('--prop_step', type=int, default=2, help='propagating steps')
    parser.add_argument('--phenomNN_variant', action='store_true', default=False, help='GCN* model.') 
    ...
    data.hyperedge_index.new_ones((data.hyperedge_index.shape[1],), dtype=torch.float32),(data.num_nodes, data.num_edges)
    ).to_dense()
    ...
    for seed in tqdm(range(args.num_seeds)):
        ...
        if args.task=="edge":
            ...
            model = PhenomNN_e(nfeat=args.num_features, args=args).to(args.device)
            ...
            model = PhenomNN_n(nfeat=args.num_features, nclass=args.num_classes,args=args).to(args.device)
    ...
    path = (f'./results/result_{args.data}_{args.method}_edge.txt'
            if args.task == "edge" else f'./results/result_{args.data}_{args.method}.txt')
    with open(path, 'a+') as write_obj:
        write_obj.write(f'{args}=> {best_test},{best_test.mean():.1f} ± {best_test.std():.1f}\n')
