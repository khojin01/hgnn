...
class EquivSetConv(nn.Module):
    ...
    def forward(self, X, vertex, edges, X0):
        ...
        Xe = scatter(Xve, edges, dim=-2, reduce=self.aggr) # [E, C], reduce is 'mean' here as default
...
class MeanDegConv(nn.Module):
    ...
    def forward(self, X, vertex, edges, X0):
        ...
        deg_e = scatter(torch.ones(Xve.shape[0], device=Xve.device), edges, dim=-2, reduce='sum')
        ...
        deg_v = scatter(torch.ones(Xev.shape[0], device=Xev.device), vertex, dim=-2, reduce='sum')
        X = self.W3(torch.cat([Xv, X, X0, torch.log(deg_v)[..., None]], -1))

        return X

class EquivSetGNN_n(nn.Module):
...
class EquivSetGNN_e(nn.Module):
    def __init__(self,args, num_features):
        ...
        self.in_channels = num_features
        self.hidden_channels = args.MLP_hidden
        self.output_channels = args.MLP_hidden
