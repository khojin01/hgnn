...
if __name__ == '__main__':
    ...
    for seed in tqdm(range(args.num_seeds)):
        ...
        if args.task=="edge":
            ...
            model = EquivSetGNN_e(args,args.num_features).to(args.device)
            ...
            model = EquivSetGNN_n(args,args.num_features,args.num_classes).to(args.device)
            optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-6)
            best_val = float('-inf')
            import time
    ...
    with open(path, 'a+') as write_obj:
        write_obj.write(f'{args.lr}rs{args.restart_alpha}=> {best_test},{best_test.mean():.1f} ± {best_test.std():.1f}\n')
