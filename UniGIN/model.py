...
class UniGIN_n(nn.Module):
    def __init__(self, args, V, E):
        ...
        self.conv_out = UniGINConv(args, self.nhid, self.nclass)
        self.convs = nn.ModuleList(
            [ UniGINConv(args, self.nfeat, self.nhid)] +
            [UniGINConv(args, self.nhid, self.nhid) for _ in range(self.nlayer-2)]
        )
        self.V = V 
        self.E = E 
        self.act = nn.ReLU()
...
class UniGIN_e(nn.Module):
    def __init__(self, args, V, E):
        ...
        self.conv_out = UniGINConv(args, self.nhid, self.nhid)
        self.convs = nn.ModuleList(
            [ UniGINConv(args, self.nfeat, self.nhid)] +
            [UniGINConv(args, self.nhid, self.nhid) for _ in range(self.nlayer-1)]
        )
        self.V = V 
        self.E = E 
        self.act = nn.ReLU()
