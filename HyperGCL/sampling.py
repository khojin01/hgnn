...
def negative_sampling(edge_index: Tensor,
    ...
    :rtype: LongTensor
    """
    assert method in ['sparse', 'dense', 'gpu']
    ...
    if method == 'dense':
        ...
        # Keep the rejection sampling entirely on the tensor's device.  The
        # legacy sparse path below calls NumPy and Python's random module on
        # every training epoch, forcing a CUDA synchronization and host copy.
        # `torch.isin` performs the membership test on CUDA without allocating
        # the full population-sized mask required by the dense method.
        for _ in range(10):
            rnd = torch.randint(population, (sample_size,), device=edge_index.device)
            mask = ~torch.isin(rnd, idx)
            if neg_idx is not None:
                mask &= ~torch.isin(rnd, neg_idx)
            rnd = rnd[mask]
            neg_idx = rnd if neg_idx is None else torch.cat([neg_idx, rnd])
            if neg_idx.numel() >= num_neg_samples:
                neg_idx = neg_idx[:num_neg_samples]
