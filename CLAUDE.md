...
`tools/discord_bot.py` 가 실험 알림과 조회를 맡는다. `refresh_all.sh` 가 매분 `notify` 로 상태 변화를
웹훅에 올리고 `ensure-bot` 으로 조회 루프를 살려 둔다. 설정은 `.discord.json` (웹훅 URL · 봇 토큰 · 채널,
저장소에 안 들어간다). 알림은 표준 라이브러리만 쓰고, 조회 루프만 `~/.venvs/discord` 의 discord.py 를 쓴다.
읽기 전용이다 — 봇으로 실험을 시작하거나 멈추지 않는다.

## 아티팩트는 두 개다

결과가 바뀌어 아티팩트를 갱신할 때는 **둘 다** 발행한다. 하나만 올리면 다른 하나가 조용히 낡는다.

- HGNN MAIN (관제) — 실행 상태·에이전트·연구 노트·지시·커밋
- HyperGC 재현 대조(dms2) — Table 3·4·5 셀 단위 Δ

서버는 두 파일을 1분마다 다시 만들지만, 발행은 claude.ai 안에서만 되는 일이라 세션이 해야 한다.
자세한 것은 `dashboard/README.md` 의 "자동화되지 않는 것".
